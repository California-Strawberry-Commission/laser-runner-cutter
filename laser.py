...
class IldaLaser:
    def __init__(self, frame_shape=(4096, 4096)): 
    ...
    def initialize(self):
        ...
        self.HeliosLib = ctypes.cdll.LoadLibrary("./include/libHeliosDacAPI.so")
    ...
    def add_pos(self, pos, color=(10, 0, 0), pad=50, intensity=1):
        ...
        print(f"sending point:{point} for pos: {pos}")
    ...
    def add_point(self, point, color=(10, 0, 0), pad=50, intensity=1):
        ...
        self.point_list.append(HeliosPoint(int(point[0]), int(point[1]), color[0], color[1], color[2], intensity))
        if pad: 
            for x in range(pad): 
                self.point_list.append(HeliosPoint(int(point[0]), int(point[1]), 0, 0, 0, 0))
    ...
    def add_square(self, center, radius, color=(10, 0, 0), pad=50):
        ...
        for x in range(-1*radius, radius+1):      
            for y in range(-1*radius, radius+1):    
                self.point_list.append(HeliosPoint(x+int(center[0]), y+int(center[1]), color[0], color[1], color[2], 0))
        if pad: 
            for x in range(100): 
                self.point_list.append(HeliosPoint(int(center[0]), int(center[1]), 0, 0, 0, 0))
    ...
    def sendFrame(self, verbose=False): 
        ...
        while (statusAttempts < 512 and status != 1):
            statusAttempts += 1
            if verbose: 
                print(f"Status check {statusAttempts} failed, status :{status}")
            status = self.HeliosLib.GetStatus(0) 

        ret = self.HeliosLib.WriteFrame(0, num_points*45, 0, ctypes.pointer(frame), num_points) #Send the frame
        self.point_list = []
