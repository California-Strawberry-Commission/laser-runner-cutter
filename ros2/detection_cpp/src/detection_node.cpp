...
class DetectionNode : public rclcpp::Node {
  ...
  public:
    ...
    void frame_callback(const camera_control_interfaces::msg::LucidFrameImages::SharedPtr msg) {
      // cv::Mat image(cv::Size(2048, 1536), CV_8UC3, const_cast<uint8_t*>(msg->color.data.data()), msg->color.step);
      // auto dptr = reinterpret_cast<void*>(msg->color_gpu_data_ptr);
      // cv::cuda::GpuMat gpuImg(msg->color_height, msg->color_width, msg->color_type, dptr, msg->color_step);

      // Start asynchronous download of the image from GPU to host while inference is running
      // cv::Mat image(cv::Size(msg->color_width, msg->color_height), CV_8UC3);
