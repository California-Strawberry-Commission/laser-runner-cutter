...
class GStreamerRosWebRTCNode(Node):
    def __init__(self):
        ...
        self.framerate = self.get_parameter('image_framerate').get_parameter_value().integer_value

        self.declare_parameter('gst_input_encoding', 'rgb8')
        self.gst_input_encoding = self.get_parameter('gst_input_encoding').get_parameter_value().string_value

        self.declare_parameter('signaling_server_url', 'ws://localhost:8081')
        self.signaling_server_url = self.get_parameter('signaling_server_url').get_parameter_value().string_value

        self.declare_parameter('our_peer_id', 123)
        self.our_peer_id = self.get_parameter('our_peer_id').get_parameter_value().integer_value

        self.declare_parameter('target_peer_id', 456)
        self.target_peer_id = self.get_parameter('target_peer_id').get_parameter_value().integer_value

        self.declare_parameter('webrtc_stun_server', 'stun://stun.l.google.com:19302')
        self.webrtc_stun_server = self.get_parameter('webrtc_stun_server').get_parameter_value().string_value
        
        self.declare_parameter('webrtc_turn_server', '')
        self.webrtc_turn_server = self.get_parameter('webrtc_turn_server').get_parameter_value().string_value
        ...
        self._glib_thread = threading.Thread(target=self._run_glib_main_loop, daemon=True)
        ...
        if state != Gst.State.PLAYING:
            self.get_logger().error(f"Pipeline failed to reach PLAYING state, current state: {state.value_nick}")
        else:
    ...
    def setup_connection_monitor(self):
        ...
        def check_connection():
            if hasattr(self, 'webrtcbin') and self.webrtcbin:
                self.webrtcbin.connect('notify::connection-state', self.on_connection_state_changed)
                self.webrtcbin.connect('notify::ice-connection-state', self.on_ice_state_changed)
            return False
        
        GLib.timeout_add(2000, check_connection)
    ...
    def ensure_media_flow(self):
        ...
        if position[0]:
            self.pipeline.seek_simple(Gst.Format.TIME, Gst.SeekFlags.FLUSH | Gst.SeekFlags.KEY_UNIT, position[1])
    ...
    def create_pipeline_manually(self):
        ...
        if not all([self.appsrc, videoconvert, queue1, queue2, queue3, encoder, 
                    h264parse, rtppay, capsfilter1, capsfilter2, capsfilter3, self.webrtcbin]):
        ...
        caps_str = f"video/x-raw,format={gst_format},width={self.width},height={self.height},framerate={self.framerate}/1"
        caps1 = Gst.Caps.from_string(caps_str)
        self.appsrc.set_property("caps", caps1)
        ...
        caps2 = Gst.Caps.from_string(f"video/x-raw,format=I420,width={self.width},height={self.height},framerate={self.framerate}/1")
        capsfilter1.set_property("caps", caps2)
        
        caps3 = Gst.Caps.from_string("video/x-h264,profile=baseline,stream-format=byte-stream")
        capsfilter2.set_property("caps", caps3)
        
        caps4 = Gst.Caps.from_string("application/x-rtp,media=video,encoding-name=H264,payload=96,clock-rate=90000")
        capsfilter3.set_property("caps", caps4)
    ...
    def _on_gst_message(self, bus, message):
        ...
        if t == Gst.MessageType.EOS:
            ...
            self.get_logger().error(f"GStreamer Error: {err.message} from {message.src.get_name()} ({debug})")
            self.glib_main_loop.quit()
        elif t == Gst.MessageType.WARNING:
            err, debug = message.parse_warning()
            self.get_logger().warning(f"GStreamer Warning: {err.message} from {message.src.get_name()} ({debug})")
        elif t == Gst.MessageType.STATE_CHANGED:
            if message.src == self.pipeline:
                old, new, pending = message.parse_state_changed()
                self.get_logger().debug(f"Pipeline state changed: {old.value_nick} -> {new.value_nick}")
        elif t == Gst.MessageType.STREAM_STATUS:
            status_type, owner = message.parse_stream_status()
            self.get_logger().debug(f"Stream status: {status_type.value_nick} for {owner.get_name()}")
    ...
    def _sigint_handler(self, signum, frame):
        self.get_logger().info("SIGINT (Ctrl+C) received. Initiating graceful shutdown...")
        rclpy.shutdown()
    ...
    def image_callback(self, msg: Image):
        ...
        if msg.encoding != self.gst_input_encoding:
            self.get_logger().warning(f"Unexpected image encoding: {msg.encoding}, expected: {self.gst_input_encoding}")
            return
        
        if msg.width != self.width or msg.height != self.height:
            self.get_logger().warning(f"Image size mismatch: {msg.width}x{msg.height}, expected: {self.width}x{self.height}")
            return
        ...
        if self.framerate > 0:
            gst_buffer.duration = Gst.SECOND // self.framerate
        else:
        ...
        if ret != Gst.FlowReturn.OK:
            if ret == Gst.FlowReturn.FLUSHING:
                ...
                self.get_logger().warning(f"Failed to push buffer to appsrc: {ret.value_name}")
        else: #debugging
            self.frame_count += 1
            if self.frame_count % 30 == 0:
                position = self.pipeline.query_position(Gst.Format.TIME)
                if position[0]:
                    actual_time = position[1] / Gst.SECOND
                    self.get_logger().info(f"Pushed {self.frame_count} frames, pipeline time: {actual_time:.2f}s")
                    
                    if self.connection_established and self.frame_count > 60:
                        if actual_time < 1.0:
                            self.get_logger().warning("Pipeline time stuck, forcing unblock")
                            self.ensure_media_flow()
                else:
                    self.get_logger().info(f"Pushed {self.frame_count} frames to pipeline")
    ...
    def destroy_node(self):
        ...
        if hasattr(self, 'glib_main_loop') and self.glib_main_loop and self.glib_main_loop.is_running():
            self.glib_main_loop.quit()
        
        if hasattr(self, '_glib_thread') and self._glib_thread and self._glib_thread.is_alive():
