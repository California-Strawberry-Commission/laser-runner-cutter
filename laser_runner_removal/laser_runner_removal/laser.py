...
class IldaLaser:
    ...
    def initialize(self):
        ...
        self.HeliosLib = ctypes.cdll.LoadLibrary("./src/laser_runner_removal/include/libHeliosDacAPI.so")
    ...
    def sendFrame(self, verbose=False): 
        ...
        while (statusAttempts < 512 and status != 1):
            statusAttempts += 1
            if verbose: 
                self.logger.debug(f"Status check {statusAttempts} failed, status :{status}")
            status = self.HeliosLib.GetStatus(0) 
