...
class RealSense(Node):
    def __init__(self): 
        ...
        #Add to config 
        self.frame_period = .1 #s
        self.frame_callback = self.create_timer(self.frame_period, self.frame_callback)
        
        self.rec_debug_frame = True

        self.frame_srv = self.create_service(RetrieveFrame, "retrieve_frame", self.retrieve_frame)
    ...
    def initialize(self): 
        #Setup yolo model 
        self.model = YOLO("./src/laser_runner_removal/include/RunnerSegModel.pt") 
    ...
    def frame_callback(self):
        ...
        self.logger.debug(f"Publishing frame ts: {frame_ts}, current time:{time.time()}")
    ...
    def draw_laser(self, debug_frame, laser_list):
        for laser in laser_list: 
            ...
            debug_frame = cv2.drawMarker(debug_frame, pos, (0, 0, 0), cv2.MARKER_CROSS, thickness=1, markerSize=20) 
        return debug_frame

    def draw_runners(self, debug_frame, runner_list):
        for runner in runner_list: 
            pos = [int(runner[0]), int(runner[1])]
            debug_frame = cv2.drawMarker(debug_frame, pos, (255, 0, 0), cv2.MARKER_STAR, thickness=1, markerSize=20) 
        return debug_frame
    ...
    def color_pixel_to_depth(self, x, y, frame):
        ...
        self.logger.debug(f"Color point:[{x} {y}] corresponding Depth point{depth_pixel}")
    ...
    def get_pos_location(self, x, y, frame): 
        ...
        self.logger.debug(f"color point:[{x}, {y}] corresponding color_pos: {pos_wrt_color}") 
