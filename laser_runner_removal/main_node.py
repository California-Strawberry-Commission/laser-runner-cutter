...
class Calibrate(State): 
    ...
    def execute(self, blackboard): 
        ...
        for point in lsr_pts: 
            ...
            blackboard.main_node.logger.debug(f"Point:{point} Laser Send Ts:{laser_send_ts}")
            while not found_pos and attempts<50: 
                blackboard.laser.add_point(point, pad=False, color = (10, 0, 0), intensity=1)
                blackboard.laser.sendFrame()
                
                attempts += 1
                last_ts, laser_data = blackboard.main_node.laser_pos_queue.get_ts(time.time())
...
class Acquire(State): 
    ...
    def __init__(self): 
        State.__init__(self,outcomes=["calibration_needed", "target_acquired", "no_target_found"])
